...
curl -s -H "Authorization: Bearer bench-northbound-token" \
  localhost:29082/v5/streams/basic/endpoints | jq
...
| 29081 | southbound (`/v5/nodes` shows registered capabilities) |
| 29082 | controller: dashboard at `/ui`, `/view`; API at `/v5/status`, `/v5/streams/{name}/endpoints` |
| 28080 | strom-1 API |
| 28081 | strom-2 API |
...
```sh
curl -s -H "Authorization: Bearer bench-southbound-token" localhost:29081/v5/nodes | jq
curl -s -H "Authorization: Bearer bench-northbound-token" localhost:29080/v5/streams | jq
```
...
and the `/v5/status` rollup need no token, so anyone who can reach port 29082
...
Both contracts live under `/v5` (see the root README). `/health` and the
...
- The controller never talks to Strom. Per stream it derives an ordered hop chain
  ...
  pull their own hops (`GET /v5/nodes/{id}/desired` via southbound, full replace of
...
- Per-stream status is rolled up from adapter-reported hop conditions:
  ...
  → `flowing`. See the controller `/v5/status` endpoint.
