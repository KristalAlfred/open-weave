...
curl -s -H "Authorization: Bearer bench-northbound-token" \
  localhost:29082/v4/streams/basic/endpoints | jq
...
| 29081 | southbound (`/v4/nodes` shows registered capabilities) |
| 29082 | controller: dashboard at `/ui`, `/view`; API at `/v4/status`, `/v4/streams/{name}/endpoints` |
| 28080 | strom-1 API |
| 28081 | strom-2 API |
...
```sh
curl -s -H "Authorization: Bearer bench-southbound-token" localhost:29081/v4/nodes | jq
curl -s -H "Authorization: Bearer bench-northbound-token" localhost:29080/v4/streams | jq
```
...
and the `/v4/status` rollup need no token, so anyone who can reach port 29082
...
Both contracts live under `/v4` (see the root README). `/health` and the
...
- The controller never talks to Strom. Per stream it derives an ordered hop chain
  ...
  pull their own hops (`GET /v4/nodes/{id}/desired` via southbound, full replace of
...
- Per-stream status is rolled up from adapter-reported hop conditions:
  ...
  → `flowing`. See the controller `/v4/status` endpoint.
