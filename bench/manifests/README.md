...
| `browser-cam-host` | camera of a page on the docker host → node-1 via the `docker-host` alias | `degraded` (see note) | — | `flowing` (see note) |
...
- **`browser-cam-host`**: `browser-cam` for a page in a browser on the docker
  host, which `just bench host-cam <node-id>` fills in. Its destination names
  node 1's `docker-host` alias, so the page is told to signal at
  `localhost:28080` while the SRT output stays on `172.26.0.10` for open-live's
  Strom (`bench/README.md`, "Feeding open-live"). Applied for the in-bench page
  it stays `pending`: that page's hop fails with `Failed to fetch`, because
  `localhost` in its container is itself. From the host, Google Chrome put H264
  640x480 and AAC on the SRT output through it, and open-live dialling that
  output took the stream to `flowing`.
